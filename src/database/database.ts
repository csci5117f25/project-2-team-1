...
export const updateTask = async (id: string, data: { last_completed_time?: number; frequency?: string; name?: string; }) => {
  const currentUser = await getCurrentUser();
  if (currentUser) {
...
export const markTaskComplete = async (id: string) => {
  ...
  if (currentUser) {
    ...
    return await addDoc(collection(db, currentUser.uid, "tasks", "completed_tasks"), completedTaskData);
  }
